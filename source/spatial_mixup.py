...
def compute_Y_and_W(grid, rotation_matrix: torch.Tensor = None, order_input: int = 1, order_output: int = 1, backend='basic', w_pattern='hypercardioid') -> torch.Tensor:
    ...
    if backend == 'basic':
        ...
        assert rotation_matrix is None or torch.all(torch.isclose(rotation_matrix, torch.eye(3).double())), 'Soundfield rotations not supported when using spatial filterbank'
...
"""
    def __init__(self, order_input: int = 1, t_design_degree: int = 3, order_output: int = 1,
        ...
        Y, W = compute_Y_and_W(self.grid, None, self.order_input, self.order_output, backend=self.backend, w_pattern=self.w_pattern)
        ...
        if G_type == 'identity':
            ...
            if capsule_center is None:
                if fnmatch.fnmatch(G_type, '*_soft'):
                    ...
                    cap_type = 'hard'
                capsule_center, capsule_width, g1_db, g2_db = draw_random_spherical_cap(spherical_cap_type=cap_type, device=self.device)
