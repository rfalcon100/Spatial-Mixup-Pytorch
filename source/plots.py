...
def plot_matrix2polar(t_matrix, order, plot_channel=0, linewidth=2, INDB=True, rlim=None, title=None, ax=None, show_plot=True, do_scaling=True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    Y_grid = spa.sph.sh_matrix(order, azis, utils.ele2colat(np.zeros_like(azis)), SH_type='real', weights=None)  # Level seems ok for input_order == 1
    ...
    _plot_polar(azis, grid_hat_azi[plot_channel,:], title=this_title, ax=axs[0], INDB=INDB, rlim=rlim, linewidth=linewidth)
    ...
    _plot_polar(eles_plot, grid_hat_ele[plot_channel,:], title=this_title, ax=axs[1], plot_elevation=True, INDB=INDB, rlim=rlim, linewidth=linewidth, rticks=4)
...
def plot_matrix2polarLinear(t_matrix, order, method='fibonacci', plot_channel=0, INDB=True, rlim=None, title=None, ax=None, show_plot=True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    eles_plot = azis - np.pi / 2
    _plot_polar(eles_plot, grid_hat_ele[plot_channel,:], title=this_title, ax=axs[1], plot_elevation=True, INDB=INDB, rlim=rlim)
    plt.tight_layout(pad=1.0, w_pad=0.0, h_pad=1.0)
    if show_plot:
        plt.show()
...
def plot_sphere_points(t_matrix, order=1, plot_channel=0, method='fibonacci', show_plot=True, fig=None, do_scaling=False, debugging=False):
